// src/pages/SalesRepLogin.jsx

import { useState } from "react";
import { salesRepLogin } from "../lib/salesRepAuth";
import { BLACK, GOLD, GOLD_DIM, WHITE, RED, DARK } from "../lib/constants";

export default function SalesRepLogin({ onSuccess }) {
  ...
  async function handleLogin() {
    ...
    setLoading(true);
    setError("");
    var result = await salesRepLogin(email, password);
    setLoading(false);
    if (result.ok) {
      onSuccess();
    } else {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 10, color: GOLD_DIM + "88", marginTop: 20 }}>
          Trimora Systems · Sales Team Access Only
        </div>
      </div>
    </div>
  );
