...
- **`REVOKE` gotcha:** Postgres grants `EXECUTE`/`SELECT` to `PUBLIC` by default at creation time. A revoke that only names a specific role (e.g. `REVOKE ... FROM anon`) can silently do nothing if `PUBLIC` still holds the grant — every role inherits through `PUBLIC` regardless. Any future grant-revocation migration on this schema should revoke from `PUBLIC` explicitly and verify afterward via `information_schema.role_routine_grants` / `role_table_grants`, not assume the migration succeeded silently. (This bit migration `044`'s first attempt, and is the same underlying issue as the `pin_reset_tokens` tautological-policy bug in `043`.)
